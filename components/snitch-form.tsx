...
export function SnitchForm({ businesses }: { businesses: Jacket[] }) {
  ...
  return <form className="mt-10 space-y-5 rounded-3xl bg-white p-6 ring-1 ring-stone-200 md:p-8">
    ...
    {creatingJacket && <fieldset className="space-y-5 rounded-2xl border border-stone-200 bg-[#f7f1e8] p-5">
      <legend className="px-2 font-bold">New Jacket details</legend>
      <label className="block font-bold">Business or service name<input name="businessName" required className="mt-2 w-full rounded-lg border border-stone-300 bg-white p-3" placeholder="Name of the business or service" /></label>
      <label className="block font-bold">Street address<input name="address" required className="mt-2 w-full rounded-lg border border-stone-300 bg-white p-3" placeholder="Street address" /></label>
      <div className="grid gap-5 sm:grid-cols-[1fr_120px]"><label className="block font-bold">City and state<input name="cityState" required className="mt-2 w-full rounded-lg border border-stone-300 bg-white p-3" placeholder="City, ST" /></label><label className="block font-bold">ZIP code<input name="zip" required inputMode="numeric" pattern="[0-9]{5}(-[0-9]{4})?" className="mt-2 w-full rounded-lg border border-stone-300 bg-white p-3" placeholder="00000" /></label></div>
      <label className="block font-bold">Phone number<input name="phone" required type="tel" className="mt-2 w-full rounded-lg border border-stone-300 bg-white p-3" placeholder="(555) 555-5555" /></label>
    </fieldset>}
    ...
    <label className="block font-bold">Evidence <span className="text-[#b85036]">(required)</span><input name="evidence" required className="mt-2 block w-full text-sm" type="file" accept="image/jpeg,application/pdf" multiple /><span className="mt-2 block text-sm font-normal text-stone-500">Attach at least one JPG photo or PDF receipt/document. PNG, screenshots, and other formats are not accepted.</span></label>
    ...
    <p className="text-center text-xs leading-5 text-stone-500">Reports publish under your CI profile after sign-in. New Jacket details and evidence are required for review before a record can be created.</p>
  </form>;
}
