...
export default function Home() {
  return <main>
    <section className="hero-grid overflow-hidden bg-[#10221e] px-4 py-6 text-white md:py-10">
      <div className="mx-auto grid min-h-[620px] max-w-6xl items-center gap-10 lg:grid-cols-[1.05fr_.95fr]">
        <div className="relative z-10 py-12 lg:py-0">
          <p className="eyebrow text-[#c8f169]">UNDERCOVER FACT POLICE</p>
          <h1 className="mt-5 max-w-3xl font-serif text-5xl font-semibold leading-[.98] tracking-tight md:text-7xl"># Put a Jacket on &apos;em. <em className="font-normal text-[#c8f169]">Intelligence, Unfiltered.</em></h1>
    ...
    <section className="mx-auto max-w-6xl px-4 py-20">
      <div className="flex flex-wrap items-end justify-between gap-5"><div><p className="eyebrow text-[#b85036]">START WITH WHAT YOU KNOW</p><h2 className="mt-3 font-serif text-4xl font-semibold">Everyday places. Better conversations.</h2></div><Link href="/offender-registry" className="font-bold underline decoration-[#b85036] decoration-2 underline-offset-4">Offender Registry</Link></div>
      <div className="mt-10 grid gap-px overflow-hidden rounded-3xl border border-stone-200 bg-stone-200 sm:grid-cols-2 lg:grid-cols-3">{categories.map(([name,description],i)=><Link key={name} href={`/offender-registry?q=${encodeURIComponent(name)}`} className="group bg-[#fffdf9] p-7 transition hover:bg-[#c8f169]"><span className="text-sm font-bold text-[#b85036]">0{i+1}</span><h3 className="mt-8 font-serif text-2xl font-semibold">{name}</h3><p className="mt-2 text-sm text-stone-600">{description}</p><span className="mt-7 block font-bold">Open file →</span></Link>)}</div>
    </section>

    <section className="bg-[#f1ede5] px-4 py-20"><div className="mx-auto max-w-6xl"><p className="eyebrow text-[#b85036]">LATEST INTELLIGENCE</p><div className="mt-3 flex items-end justify-between"><h2 className="font-serif text-4xl font-semibold">Files worth knowing</h2><Link href="/tea" className="hidden font-bold underline md:block">Open the intelligence hub</Link></div><div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{businesses.map(b=><BusinessCard key={b.id} business={b}/>)}</div></div></section>
    ...
    <section id="story" className="bg-[#b85036] px-4 py-20 text-[#fffaf2]"><div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[.7fr_1.3fr]"><div><p className="eyebrow text-[#ffd4a7]">OUR MISSION / HOW WE OPERATE</p><h2 className="mt-4 font-serif text-5xl leading-none">No filters. Just the facts.</h2></div><div className="space-y-6 text-lg leading-8 text-white/90"><p>Undercover Fact Police is the community intelligence desk for local business decisions. We built it because people deserve a record that is more useful than a popularity contest and more honest than a corporate review feed.</p><p>This is not a star parade. It is a place to log the paperwork: what happened, what worked, what did not, and what the next person needs to know before they walk in.</p><p>Our mission is simple: give the community a clean line to the facts. File useful intelligence, keep Jackets accurate, and help everyone make better calls.</p></div></div></section>
  </main>;
}
