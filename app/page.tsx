...
export default function Home() {
  return <main>
    <section className="hero-grid overflow-hidden bg-[#10221e] px-4 py-6 text-white md:py-10">
      <div className="mx-auto grid min-h-[620px] max-w-6xl items-center gap-10 lg:grid-cols-[1.05fr_.95fr]">
        <div className="relative z-10 py-12 lg:py-0">
          ...
          <h1 className="mt-5 max-w-3xl font-serif text-5xl font-semibold leading-[.98] tracking-tight md:text-7xl">Share your two cents. <em className="font-normal text-[#c8f169]">Help businesses</em> get better.</h1>
          <p className="mt-7 max-w-xl text-lg leading-8 text-white/75">Welcome to My Two ¢ — the place where real customers leave honest reviews and unsolicited advice for the businesses they use every day. Retail, restaurants, beauty, auto, medical, events, and more. Speak up. Be useful. Make a difference.</p>
    ...
    <section className="bg-[#f1ede5] px-4 py-20"><div className="mx-auto max-w-6xl"><p className="eyebrow text-[#b85036]">WHAT PEOPLE ARE TALKING ABOUT</p><div className="mt-3 flex items-end justify-between"><h2 className="font-serif text-4xl font-semibold">Worth your two cents</h2><Link href="/search" className="hidden font-bold underline md:block">See every review</Link></div><div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{businesses.map(b=><BusinessCard key={b.id} business={b}/>)}</div></div></section>

    <section id="story" className="bg-[#b85036] px-4 py-20 text-[#fffaf2]"><div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[.7fr_1.3fr]"><div><p className="eyebrow text-[#ffd4a7]">OUR STORY / HOW WE GOT STARTED</p><h2 className="mt-4 font-serif text-5xl leading-none">No fluff. No corporate filter.</h2></div><div className="space-y-6 text-lg leading-8 text-white/90"><p>Hey there! We’re the folks behind My Two ¢ — a small but passionate crew who got tired of the usual review sites that felt more like popularity contests than real talk. We started this because consumers deserved a place to share honest opinions, practical advice, and unsolicited two cents with the businesses they actually use every day.</p><p>Not just star ratings and “great service!” comments, but useful feedback: what works, what doesn’t, and what would bring you back. We built My Two ¢ as a Glassdoor-style space for consumer goods and services — a place where everyday people can speak up, help businesses improve, and help other customers make smarter choices.</p><p>Our mission is simple: give consumers a stronger voice and give businesses the honest feedback they need to get better. Come share your two cents. The world — and that business around the corner — will be better for it.</p></div></div></section>
  </main>;
}
